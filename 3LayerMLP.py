...
for i in range(epochs):
    ...
    o2 = np.zeros((l2_neurons, l3_neurons))
    ...
    for j in range(np.shape(x_train)[0]):
        ...
        # Back propagation
        f1_driviate = sigmoid_deriviate(net1)
        w2_f_deriviate = np.matmul(f1_driviate, w2)
        ...
        f2_driviate = sigmoid_deriviate(net2)
        w3_f_deriviate = np.matmul(f2_driviate, w3)
        w3_f_deriviate_x = np.matmul(w3_f_deriviate, np.transpose(np.reshape(np.transpose(o1), (-1, 1))))
        w2 = np.subtract(w2, np.transpose((eta * err * -1 * 1 * w3_f_deriviate_x)))
